$('document').ready(function () {
    ...
    function appendItemsToPage(beerArr) {
        ...
        for (var i = currentPage * 52; i < currentPage * 52 + 52 && i < beerArr.length; i++) {
    ...
    var search = function () {
        ...
        searchText = $('.search-text-box').get(0).value;
        var minAbvText = $('.minabv-text-box').get(0).value;
        var maxAbvText = $('.maxabv-text-box').get(0).value;
        $.ajax({
            url: beerApi + '?name=' + searchText + "&minAbv=" + minAbvText + "&maxAbv=" + maxAbvText,
            method: 'GET',
        })
            .done(function (response) {
                beerArr = response;
